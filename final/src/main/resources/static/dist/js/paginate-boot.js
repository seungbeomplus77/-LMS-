 ...
 */
const pageCount = function(dataCount, pageSize) {
	let result = 0;
...
/*
  - 화면에 표시할 페이지를 중앙에 출력
	...
	url : 링크를 설정할 url
*/
const pagingUrl = function(current_page, total_page, url) {
	let result = '';
...
*/
const pagingMethod = function(current_page, total_page, methodName) {
	let result = '';
    ...
    try {
        ...
        while(page <= total_page && page < n + numPerBlock) {
            if(page === current_page) {
            	...
            	result += '<li class="page-item"><a class="page-link" onclick="' + methodName + '(' + page + ');">' + page + '</a></li>';
            }
