...
<div class="category-buttons mb-3 mt-4">
  <div class="btn-group d-flex" role="group" aria-label="입학안내 카테고리">
    <c:url var="urlAll" value="/gradeGuide/main">
      <c:param name="categoryId" value="1" />
    </c:url>
    <c:url var="urlNew" value="/gradeGuide/main">
      <c:param name="categoryId" value="2" />
    </c:url>
    <c:url var="urlTransfer" value="/gradeGuide/main">
      <c:param name="categoryId" value="3" />
    </c:url>
    <c:url var="urlSpecial" value="/gradeGuide/main">
    ...
    <a href="${pageContext.request.contextPath}${urlTransfer}" class="btn btn-outline-secondary flex-fill rounded-0 fw-bold">성적산출방법</a>
   	<a href="${pageContext.request.contextPath}/gradeGuide/main?categoryId=4" class="btn btn-outline-secondary flex-fill rounded-0 fw-bold">이수기준/졸업요건</a>
...
<table class="table">
  ...
  <tbody>
    ...
    <tr>
    	...
    	<td>
			<a href="${pageContext.request.contextPath}/gradeGuide/article/${dto.gradeGuideNum}?page=${page}${not empty query ? '&' : ''}${query}" class="text-reset">
			    ${dto.subject}
			</a>
    	</td>
    	<td>
...
<script type="text/javascript">
  ...
  function searchList() {
    ...
    const url = '${pageContext.request.contextPath}/gradeGuide/main';
