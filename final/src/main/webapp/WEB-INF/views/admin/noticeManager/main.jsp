...
<div class="category-buttons mb-3 mt-4">
  <div class="btn-group d-flex" role="group" aria-label="입학안내 카테고리">
    <c:url var="urlAll" value="/admin/noticeManager/main">
      <c:param name="categoryId" value="1" />
    </c:url>
    <c:url var="urlNew" value="/admin/noticeManager/main">
      <c:param name="categoryId" value="2" />
    </c:url>
    <c:url var="urlTransfer" value="/admin/noticeManager/main">
      <c:param name="categoryId" value="3" />
    </c:url>
    <c:url var="urlHappy" value="/admin/noticeManager/main">
    ...
    <a href="${pageContext.request.contextPath}${urlTransfer}" class="btn btn-outline-secondary flex-fill rounded-0 fw-bold">생활/규정</a>
   	<a href="${pageContext.request.contextPath}/admin/noticeManager/main?categoryId=4" class="btn btn-outline-secondary flex-fill rounded-0 fw-bold">장학/복지</a>
...
<tbody>
  ...
  <tr>
    ...
    <td>
      <a href="${pageContext.request.contextPath}/admin/noticeManager/article/${dto.schoolNoticeNum}?page=${page}${not empty query ? '&' : ''}${query}" class="text-reset">
        ${dto.subject}
      </a>
    </td>
    <td>${dto.reg_date}</td>
...
<div class="col text-end">
  <button type="button" class="btn btn-primary" onclick="location.href='${pageContext.request.contextPath}/admin/noticeManager/write';">게시글 등록</button>
</div>  
...
<script type="text/javascript">
  ...
  function searchList() {
    ...
    const url = '${pageContext.request.contextPath}/admin/noticeManager/main';
