...
<head>
  ...
  <script type="text/javascript">
    function sendOk() {
        ...
        f.action = '${pageContext.request.contextPath}/admin/noticeManager/${mode}';
        f.submit();
    }
    
    <c:if test="${mode == 'update'}">
    function deleteFile(schoolNoticeNum) {
        if( ! confirm('파일을 삭제하시겠습니까?') ) {
            return;
        }
        let url = '${pageContext.request.contextPath}/admin/noticeManager/deleteFile?schoolNoticeNum=' + schoolNoticeNum + '&page=${page}';
        location.href = url;
    }
    </c:if>
  </script>
      ...
      <form name="boardForm" action="" method="post" enctype="multipart/form-data">
        ...
        <div class="form-group mb-3">
          ...
          <select name="categoryId" id="categoryId" class="form-control">
            <option value="2" <c:if test="${dto.categoryId == 2}">selected</c:if>>진로/입시</option>
            <option value="3" <c:if test="${dto.categoryId == 3}">selected</c:if>>생활/규정</option>
            <option value="4" <c:if test="${dto.categoryId == 4}">selected</c:if>>장학/복지</option>
            <!-- 추가 옵션 -->
          </select>
          ...
          <div class="form-group mb-3">
            ...
            <c:if test="${not empty dto.saveFilename}">
              <p class="form-control-plaintext">
                <a href="javascript:deleteFile('${dto.schoolNoticeNum}')"><i class="bi bi-trash"></i></a>
        ...
        <div class="form-group">
          ...
          <a href="${pageContext.request.contextPath}/admin/noticeManager/main" class="btn btn-link">
            <c:choose>
          ...
          <c:if test="${mode == 'update'}">
            <input type="hidden" name="schoolNoticeNum" value="${dto.schoolNoticeNum}">
